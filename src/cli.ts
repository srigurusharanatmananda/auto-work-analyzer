  ...
  .action(async (options) => {
    try {
      ...
      if (options.tasks !== false) {
        ...
        const tasks = await analyzer.createTasksFromWork(
          workAnalysis,
          config.clickup
        );
 ...
 */
function displayAnalysisResults(workAnalysis: any, format: string) {
  if (format === "json") {
  ...
  if (workAnalysis.detectedWork.length > 0) {
    console.log("\n📋 Detected work:");
    workAnalysis.detectedWork.forEach((work: any, index: number) => {

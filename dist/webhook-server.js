...
export async function startWebhookServer(port = 3000) {
    try {
                ...
                res.json({
                    ...
                    data: {
                        ...
                        summary: {
                            ...
                            totalLinesChanged: workAnalysis.totalLinesAdded + workAnalysis.totalLinesDeleted,
        ...
        app.post("/notes", upload.single("notes"), async (req, res) => {
            try {
                ...
                res.json({
                    ...
                    message: `Processed ${processedNotes.tasks.length} tasks from notes${createTasks ? `, created ${createdTasks.length} ClickUp tasks` : ""}`,
                });
            }
            catch (error) {
        ...
        app.post("/webhook", async (req, res) => {
            try {
                const { type, project, date, endDate, author, branch, repository, commitHash, secret, } = req.body;
                ...
                // Analyze work
                const workAnalysis = await analyzer.analyzeWork(analysisDate, analysisEndDate, author);
        ...
        // Start server
        app.listen(port, () => {
            console.log(`🚀 Auto Work Analyzer webhook server running on port ${port}`);

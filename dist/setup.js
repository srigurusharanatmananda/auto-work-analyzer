...
export async function interactiveSetup() {
    ...
    try {
        ...
        console.log("------------------------");
        const projectName = (await question("Project name: ")) || "Auto Work Analyzer";
        const projectDescription = (await question("Project description: ")) ||
            "Automatic work analysis and task creation";
        const projectPath = (await question(`Project path (${process.cwd()}): `)) || process.cwd();
        console.log("");
...
`;
    ...
    if (existsSync(gitHooksDir)) {
        ...
        // Make it executable
        require("child_process").execSync(`chmod +x ${join(gitHooksDir, "post-commit")}`);
...
export function generateCronJob() {
    ...
    const scriptPath = join(projectPath, "node_modules", ".bin", "auto-work-analyzer");

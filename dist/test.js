 ...
 */
async function testClickUpConnectivity() {
    try {
        ...
        console.error("❌ ClickUp connectivity test failed:", error instanceof Error ? error.message : "Unknown error");
        return false;
    }
}
/**
...
async function testGitRepository() {
    try {
        ...
        console.error("❌ Git repository test failed:", error instanceof Error ? error.message : "Unknown error");
        return false;
    }
}
/**
 * Test work analysis
 */
async function testWorkAnalysis() {
    try {
        ...
        console.error("❌ Work analysis test failed:", error instanceof Error ? error.message : "Unknown error");
        return false;
    }
}
/**
...
async function testTaskCreation() {
    try {
        ...
        if (workAnalysis.detectedWork.length > 0) {
            ...
            console.log(`📋 Would create ${workAnalysis.detectedWork.length + 1} tasks (1 summary + ${workAnalysis.detectedWork.length} individual)`);
        }
        else {
            console.log("ℹ️  No tasks would be created (no significant work detected)");
        }
        return true;
    }
    catch (error) {
        console.error("❌ Task creation test failed:", error instanceof Error ? error.message : "Unknown error");
        return false;
    }
}
/**
